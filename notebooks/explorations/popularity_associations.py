...
def _(EvidenceStatus, KNNImputer, NarrativeSection, SimpleImputer, confirmation, fit_clustered_ols, human, mo, np, pl, render_narrative_section, synthetic_imputation_metrics, tracks):
    ...
    imputation_seed = 2026
    imputation_population = confirmation.sort("track_id").head(2_000)
    imputation_population_n = imputation_population.height
    rng = np.random.default_rng(imputation_seed)
    observed = imputation_population.select(human).to_numpy().astype(float)
    mask = rng.random(observed.shape) < 0.15
