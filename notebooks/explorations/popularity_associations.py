...
def _():
    ...
    local_root = next((candidate for start in (Path.cwd(), *Path(__file__).resolve().parents) for candidate in (start, *start.parents) if (candidate / ".git").exists() and (candidate / "pyproject.toml").exists() and (candidate / "spotify_data").exists()), None)
