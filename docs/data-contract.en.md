...
| `tracks_clean` | one cleaned source row without the exported index | shared input for every analytical relation |
...
`tracks_clean` reproduces the cleaning audited in the [Spotify Data Cleaning](https://molab.marimo.io/notebooks/nb_FP1VJkrCoXJwRVExDxZibv) notebook: it removes the single row missing `artists`, `album_name`, and `track_name`; omits `source_row_id` from the analytical relation; removes 450 exactly duplicated copies after excluding that index; and trims outer whitespace from `artists` and `track_name`. The source remains unchanged in `tracks_raw`. The result has 113,549 rows and 20 columns. Do not persist a second cleaned CSV; every notebook must rebuild this relation through the same module.
...
- cardinality changes from `tracks_raw` to `tracks_clean`, `tracks`, and `track_genres`;
